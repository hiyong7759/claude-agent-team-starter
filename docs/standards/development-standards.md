...
> **Purpose:** Define development/maintenance standards for Python scripts and Markdown personas aligned with "File-Based Handoff" architecture and "Strict Governance".
...
### 1.2 Python First & Scripts

Functional mechanisms of the system are implemented as **Python scripts**.
...
### 1.4 Project Directory Structure (Isolation)

Clearly separate agent system (Engine) and target project (Payload).

```
workspace/
├── .claude/                 # System: Agent tools and runtime (scripts/config)
├── docs/
│   ├── standards/          # System: Standard documents
│   └── projects/           # Project: Project-specific planning docs
│       └── {ProjectID}/
└── projects/               # Project: Actual product source code
    └── {ProjectID}/
        ├── src/
        └── tests/
```
...
### 5.1 Workspace Setup (Portability)

- **Multi-Repo:** Agent system (`ws/`) and each project (`ws/projects/*`) managed as independent repositories.
- **Automated Restore:** Use `setup_workspace.py` (root wrapper) or `.claude/scripts/setup_workspace.py` to batch restore (Clone/Pull) all projects registered in `docs/registry/project-registry.md`.
- **Migration Workflow:** Set up entire development environment in just 2 steps when setting up new environment: `Agent Repo Clone` -> `Script Run`.
...
# Review HTML coverage report (if available)
...
**Co-located Tests (Preferred for Projects):**
```
projects/{ProjectID}/
├── src/
│   ├── utils/
...
**Centralized Tests (Preferred for System Scripts):**
```
.claude/
├── scripts/
│   ├── context_manager.py
...
**Rationale:**
- Co-location: Easier to find related tests, natural organization
- Centralized: Clearer separation of test infrastructure, preferred for Python scripts
...
- `src/utils/helpers.test.ts` (lines 1-45)
- `src/api/client.test.ts` (lines 23-67)
